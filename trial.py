...
file_path = r"C:\Users\vanis\OneDrive\Desktop\TRADEMAN\TradeMan\UserProfile\excel\sridhar.xlsx"
...
def calculate_and_print_tax_from_excel(file_path):
    columns = ["Tr No", "Strategy", "Stock", "Date", "Entry Time", "Exit Time",
               "Entry Price", "Exit Price", "Hedge Entry", "Hedge Exit", "Trade points", "Qty", "PnL", "Taxes", "Net PnL", "Peak Margin"]

    # Read the excel file
    df = pd.read_excel(
        file_path, sheet_name='Overnight_Stocks', names=columns, header=1)
    ...
    for index, row in df.iterrows():
        ...
        # Calculate tax using aliceblue_taxes
        tax = aliceblue_taxes(qty,  orders, hedge_entry, hedge_exit)
        futures_tax = aliceblue_futures_taxes(qty, orders, entry_prc, exit_prc)
...
# Sample usage:
file_path = r"C:\Users\vanis\OneDrive\Desktop\Trademan\Trading excel files\929016(Brijesh).xlsx"
calculate_and_print_tax_from_excel(file_path)
