...
def save_all_sheets_to_excel(all_dfs, excel_path):
    # Define a date format style for openpyxl
    date_style = NamedStyle(name='datetime', number_format='DD/MM/YYYY')
    # Define a time format style for openpyxl
    ...
    with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
        for sheet_name, df in all_dfs.items():
            ...
            for row in worksheet.iter_rows():
                for cell in row:
                    # Apply date format style if the cell contains a date
                    if isinstance(cell.value, datetime.date) and not isinstance(cell.value, datetime.datetime):
                        cell.style = date_style
                    # Apply time format style if the cell contains a time value
                    elif isinstance(cell.value, datetime.time) or (isinstance(cell.value, datetime.datetime) and cell.value.date() == datetime.date(1899, 12, 31)):
...
def build_message(user, mpwizard_pnl, amipy_pnl, overnight_pnl, gross_pnl, tax, current_capital, expected_capital):
    ...
    if overnight_pnl != 0:
        message_parts.append(
            f"Overnight Options: {custom_format(overnight_pnl)}")
...
def update_excel_data(all_dfs, mpwizard_df, amipy_df, overnight_df):
    if not mpwizard_df.empty:
        all_dfs["MPWizard"] = pd.concat(
            [all_dfs.get("MPWizard", pd.DataFrame()), mpwizard_df])
    if not amipy_df.empty:
        all_dfs["AmiPy"] = pd.concat(
            [all_dfs.get("AmiPy", pd.DataFrame()), amipy_df])
    if not overnight_df.empty:
        all_dfs["Overnight_options"] = pd.concat(
            [all_dfs.get("Overnight_options", pd.DataFrame()), overnight_df])
...
def send_telegram_message(phone_number, message):
    session_filepath = os.path.join(
        script_dir, "..", '..', '..', "+918618221715.session")
...
def main():
    ...
    for broker, user in user_list:
        ...
        mpwizard_df, mpwizard_pnl, mpwizard_tax = process_strategy_data(
            user_data, broker, "MPWizard", sc.process_mpwizard_trades)
        amipy_df, amipy_pnl, amipy_tax = process_strategy_data(
            user_data, broker, "AmiPy", sc.process_amipy_trades)  # Implement a function that processes Amipy trades
        overnight_df, overnight_pnl, overnight_tax = process_strategy_data(
            user_data, broker, "Overnight_Options", sc.process_overnight_options_trades)
        ...
        message_parts = build_message(
            user, mpwizard_pnl, amipy_pnl, overnight_pnl, gross_pnl, tax, current_capital, expected_capital)
        message = "\n".join(message_parts).replace('\u20b9', '₹')
        print(message)

        update_json_data(data, broker, user, net_pnl,
                         expected_capital, broker_filepath)
